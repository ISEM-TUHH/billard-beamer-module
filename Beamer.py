...
class Beamer(Module):
	...
	def overrule_warning(self):
		...
		This is usually prohibited, as it is mostly the result of a user error.
		Raises the flag `passUnitMatrixWarning` to true.

		Returns:
			str: "You have overwritten the safety check to not write an unit matrix as the transformation matrix."
		"""
	...
	def save_transformation_matrix(self):
		"""Save the current transformation matrix for persistency.

		After a calibration, the applied matrix is not automatically stored.
		Writes to `self.transformPath` into the json field `transformation-matrix`.
		If the current matrix is a unit matrix, don't save it (see Beamer.overrule_warning) and return an error message
	...
	def configure_camera_answer(self):
		"""Use the camera module to configure the beamer output.

		This gets called by the camera module after it started looking for the beamer markers (see Beamer.start_camera_calibration).
		Based on the positions of 4 ArUco markers, the transformation between the (calibrated) camera and the (uncalibrated) beamer is calculated and applied.
		Arguments are passed as http url arguments: x1, y1, ..., x4, y4 as each markers center respectively.

		This function has been partially generated by ChatGPT-4.1 on 11.2.26.
		
		Returns:
			str: message containing the new transformation matrix.
		"""
	...
	def current_image(self):
		"""Get the image that is currently displayed before transformation.

		Returns:
			Response: image/jpg mimetype containing the current image.
		"""
